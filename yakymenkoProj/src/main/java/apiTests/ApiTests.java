...
//                new PostDTO("test2", "test body2", "All Users", "no", new AuthorDTO("autoapi"), false)
        ...
        for (int i = 0; i < expectedResult.length; i++) {
            softAssertions.assertThat(responseBody[i])
                    .isEqualToIgnoringGivenFields(expectedResult[i], "id", "createdDate", "author"); // проігноруй 3 поля, а інші порівняй
...
//        https://api.privatbank.ua/p24api/pubinfo?exchange&json&coursid=11
    ...
    public void getAllPostsByUserPath(){
        ...
        List<String> actualTitleList = actualResponse.jsonPath().getList("title", String.class); // не знаємо розмірність, тому використовуємо ліст
        SoftAssertions softAssertions = new SoftAssertions();
        for (int i = 0; i < actualTitleList.size(); i++) {
            softAssertions.assertThat(actualTitleList.get(i)).as("Item number " + i).contains("test"); // assertThat -разні перевірки
        }

        List<Map> actualAuthorList = actualResponse.jsonPath().getList("author", Map.class); // перевірка що у "author" є: → "username": "autoapi",
